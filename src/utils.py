...
def generate_wordcloud(_config, labels, cluster_id):
    # Opening JSON file
    user_mapping = json.load(open(_config['DATA']['IdMappingPath']))
    user_category_frequ = load_normalized_category_frequencies(_config['DATA']['UserCategoriesPath'], user_mapping)
    ...
    return WordCloud().generate_from_frequencies(freq)
